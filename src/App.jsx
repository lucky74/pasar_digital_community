    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                // Smart Compression: Use provided maxWidth
...
const AddProductModal = ({ onClose, user, showToast, t, CATEGORY_KEYS }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Check for specific database error (Missing Column)
            if (msg.includes('column "discount"') || msg.includes('does not exist')) {
                msg = "Database belum update! Jalankan file 'add_discount_column.sql' di Supabase.";
            } else if (msg.includes('is aborted')) {
                msg = "Koneksi lambat/terputus. Foto telah diperkecil, coba lagi.";
            }
            
            showToast(t('alert_upload_fail') + msg, 'error');
        } finally {
            setUploading(false);
        }
    };
            ...
            if (error) {
                ...
                if (data.user) {
                    ...
                    if (profileError) {
                        ...
                        // Welcome message now handled by Supabase Trigger (server-side)
